...
import { updateVehicleHealth } from './vehicleController';
...
export const createMaintenance = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await updateVehicleHealth(vehicleId);
    ...
    res.status(500).json({ message: 'Error creating maintenance service', error: (error as Error).message });
  }
};
...
export const updateMaintenance = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Error updating maintenance log', error: (error as Error).message });
  }
};
...
export const deleteMaintenance = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const log = await Maintenance.findByIdAndDelete(id);
    if (!log) {
      res.status(404).json({ message: 'Maintenance record not found' });
      return;
    }
    ...
    await updateVehicleHealth(log.vehicle.toString());
