...
export const getDashboardData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const available = await Vehicle.countDocuments({ ...query, status: 'Available' });
    const onTrip = await Vehicle.countDocuments({ ...query, status: 'In Trip' });
    const inShop = await Vehicle.countDocuments({ ...query, status: 'In Shop' });
    const retired = await Vehicle.countDocuments({ ...query, status: 'Retired' });
    ...
    const fleetValueRaw = await Vehicle.aggregate([
      { $match: query },
    ...
    const activeVehiclesList = await Vehicle.find(query).select('_id');
    ...
    const typesGroup = await Vehicle.aggregate([
      { $match: query },
    ...
    res.status(500).json({ message: 'Error retrieving dashboard analytics', error: (error as Error).message });
  }
};
